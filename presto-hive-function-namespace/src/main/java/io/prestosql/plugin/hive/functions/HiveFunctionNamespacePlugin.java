        ...
        implements Plugin
{
    ...
    {
        File path = new File(FUNCTION_PROPERTIES_FILE_PATH);
        List<URL> urls = new ArrayList<>();
        for (File file : path.listFiles()) {
            if (file.isFile() && file.length() != 0) {
                try {
                    Map<String, String> config = loadPropertiesFrom(format(FUNCTION_PROPERTIES_FILE_PATH + "%s" + file.getName(), File.separatorChar));
                    if (config.keySet().contains(EXTERNAL_FUNCTIONS_DIR)) {
                        urls = getURLs(config.get(EXTERNAL_FUNCTIONS_DIR));
                        break;
                    }
                }
                catch (IOException e) {
                    log.error("Read configuration %s fail.", file.getName());
                }
            }
        }
