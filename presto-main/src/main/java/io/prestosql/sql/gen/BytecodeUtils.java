...
import static io.prestosql.spi.function.ScalarImplementationChoice.ReturnPlaceConvention.STACK;
...
public final class BytecodeUtils
{
    ...
    public static BytecodeNode generateInvocation(Scope scope, String name, ScalarFunctionImplementation function, Optional<BytecodeNode> instance, List<BytecodeNode> arguments, CallSiteBinder binder)
    {
        ...
        for (ScalarImplementationChoice currentChoice : choices) {
            ...
            for (int i = 0; i < arguments.size(); i++) {
                ...
                if (!(arguments.get(i) instanceof InputReferenceNode) && currentChoice.getArgumentProperty(i).getNullConvention() == ScalarImplementationChoice.NullConvention.BLOCK_AND_POSITION) {
                    isValid = false;
                    break;
                }
            }
        ...
        while (currentParameterIndex < methodType.parameterArray().length) {
            ...
            if (bestChoice.getInstanceFactory().isPresent() && !boundInstance) {
                ...
                ScalarImplementationChoice.ArgumentProperty argumentProperty = bestChoice.getArgumentProperty(realParameterIndex);
    ...
    public static List<ScalarImplementationChoice> getAllScalarFunctionImplementationChoices(ScalarFunctionImplementation function)
    {
        ...
        return ImmutableList.of(new ScalarImplementationChoice(
                function.isNullable(),
                function.getInvocationConvention().getArgumentConventions().stream().map(ScalarImplementationChoice.ArgumentProperty::valueTypeArgumentProperty).collect(toImmutableList()),
                STACK,
